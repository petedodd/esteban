MODULE random
  ! P.J. Dodd 2015 p.j.dodd@sheffield.ac.uk
  ! This is a minimal adaptation of Alan Miller's fortran random number routines
  ! to use R's random number streams
...
interface                       !PJD write interface to R routines
   double precision function normrnd()
   end function normrnd
   double precision function unifrnd()
   end function unifrnd
end interface
...
CONTAINS
  ...
  FUNCTION random_normal() RESULT(fn_val)
    ! PJD version using R's normal random number
    REAL :: fn_val
    fn_val = normrnd()
    RETURN
  ...
  ! my  changed version using ranlibs implementation of same algorithm
  FUNCTION random_Poisson2(mu) RESULT(ival)
    implicit none
    integer( kind = 4 ) :: ival
