...
With `west=1` the WHO estimates are also plotted (red lines). N.B. WHO mortality shown is TB deaths in HIV-negative people only.
...
```{r, eval=TRUE,fig.width=7, fig.height=7,fig.pos="H"}
testocsp <- condsimmer(dat=P$dat,n=1,P,mappar$par,show=TRUE,truedat=NULL,V=P$V,
                        west=1,stoch=0,coarse = 1) #n=1 deterministic version
```

`truedat` is used to display data that has not been conditioned on.
...
todo: pairs, then plot

Supplying a `data.frame` of parameters overrides `n` to be the number of runs.
