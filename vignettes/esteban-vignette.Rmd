...
<!-- ### Abstract -->
<!-- > This vignette illustrates progress on the `esteban` package, contains links and instructions for installation, and serves as an interim milestone. -->
...
![Overview model structure](figs/TBests3.pdf)
...
A user must first install an up-to-date version of R.

The model dependencies should be installed first by typing

```{r,eval=FALSE}
deps <- c('ggplot2','data.table','scales') #needed
pkgs <- installed.packages()[,"Package"] #got
if(!all(deps %in% pkgs)) install.packages(deps[!(deps %in% pkgs)]) #add
```

at the R command prompt (or copy-pasting this code).

The R package is currently available in binary form from these hyperlinks: [Windows](https://drive.google.com/file/d/0B1D72Lz_kk3oR1o2TjlxSTZIS2c), [Linux](https://drive.google.com/file/d/0B1D72Lz_kk3oeGN5Tkw2MW8zWjA), [Mac OS X (<10.8)](https://drive.google.com/file/d/0B1D72Lz_kk3oQ3RkMnh0eUJGdkE). These files are either zip, tar.gz or tgz files and can be installed by typing

```{r,eval=FALSE}
install.packages('esteban_xxx.zip',repos=NULL)
```

from the same working directory as the package file. Here, the file name should be altered to match the version downloaded. Alternatively, most R GUIs include a menu-driven method to install packages from a file.
...
library(ggplot2) # for some plots exx below
library(reshape2) # for reshaping data below
```
...
The model also requires a list of parameters relevant to TB:


```{r, eval=TRUE}
tbp <- list(
  foi = 1e-2,                          #initial foi - parametrizes intiial state
  bet = 5,                             #transmission coefficient
  v = .2,                              #protection (0=complete protection)
  pr=.1,                               #primary progression (probability)
  eps = 5e-3,                          #endogenous activation (rate per year)
  cdr0 = .5,                           #initial CDR (really probability of detection)
  dcdr = .05,                          #drift in CDR (per year)
  theta = 10e0,                        #precision for noise in CDR (random walk w/ drift)
  CFRu = .5,                           #untreated CFR (really probability)
  CFRd = .05,                          #treated CFR
  TSu = 3,                             #duration untreated
  TSd = .5,                            #duration treated
  VR = .7                              #initial VR CDR (probability of death being in VR)
)
...
![South Africa: total population. Red line model; black dots UNPD data.](figs/ZAFchk_totpop.pdf)

![South Africa: demographic snapshot in 2014. Fine pyramid model output; black lines UNDP 5-year demographic snapshot.](figs/ZAFchk_demog.pdf)

![South Africa: HIV prevalence by time. Red line model; black dots AIM data.](figs/ZAFchk_hivp.pdf)

![South Africa: HIV prevalence by age.](figs/ZAFchk_agehiv.pdf)

![South Africa: ART coverage by time. Red line model; black dots UNAIDS data.](figs/ZAFchk_artp.pdf)

![South Africa: CD4 and ART status by age.](figs/ZAFchk_agecd4.pdf)
